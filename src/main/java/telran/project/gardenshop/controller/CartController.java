...
public class CartController {
        ...
        @Operation(summary = "Get current user's cart", description = "Returns the current user's shopping cart")
        @ApiResponses({
                        @ApiResponse(responseCode = "200", description = "cart get", content = @Content(schema = @Schema(implementation = CartResponseDto.class)))
        })
        @GetMapping
        ...
        @Operation(summary = "Remove cart item", description = "Remove cart item with cartItemId")
        @ApiResponses({
